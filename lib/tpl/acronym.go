...
// https://en.wiktionary.org/wiki/Template:acronym_of
type Acronym struct {
...
func (tpl *Template) ToAcronym() Acronym {
	a := Acronym{}
	tpl.toConcrete(reflect.TypeOf(a), reflect.ValueOf(&a))
	return a
}

func (a *Acronym) Text() string {
	var defn string
	if a.Alt != "" {
		defn = a.Alt
	} else {
	...
	return fmt.Sprintf("acronym of %s%s", defn, gloss)
}
